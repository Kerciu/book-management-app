...
class UserRegisterSerializerTest(TestCase):
    ...
    def test_email_exists(self):
        ...
        serializer = UserRegisterSerializer(data=data)
        self.assertFalse(serializer.is_valid())
        self.assertEqual(serializer.errors["email"][0], "This email is already in use")
...
class UserLoginSerializerTest(TestCase):
    ...
    def test_valid_login(self, mock_authenticate):
        ...
        self.assertIn("refresh_token", serializer.data)
...
class SetNewPasswordSerializerTests(TestCase):
    ...
    def test_valid_password_reset(self):
        ...
        self.assertTrue(serializer.is_valid())
        user = serializer.validate(self.valid_data)
    ...
    def test_password_mismatch(self):
        ...
        self.assertEqual(str(cm.exception.detail), "Passwords do not match")
...
class GoogleSignInSerializerTests(TestCase):
    @patch("authentication.oauth.GoogleAuth.validate")
    def test_valid_google_auth(self, mock_validate):
        ...
        user = serializer.validate_access_token("valid_google_token")
...
class GithubSignInSerializerTests(TestCase):
    @patch("authentication.oauth.GithubAuth.retrieve_user_info")
    @patch("authentication.oauth.GithubAuth.exchange_code_for_token")
    def test_valid_github_auth(self, mock_exchange, mock_retrieve):
        ...
        user = serializer.validate_code("valid_code")
    ...
    def test_missing_email(self, mock_exchange, mock_retrieve):
        ...
        serializer = GithubSignInSerializer(data=data)
        with self.assertRaises(serializers.ValidationError):
            serializer.validate_code("valid_code")
